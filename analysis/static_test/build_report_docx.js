...
const body = [
  ...
  ...numbered([
    ...
    '**The ruler-side node\'s sensor fails after ~20 minutes of running.** Its reads start hanging and returning garbage, and a few minutes later it stops producing data. Seen twice, both times with the board untouched.',
  ...
  bullet('**Longest clean hold so far:** the first ~18 minutes of the 40-minute run held at 0.80° with 0.016° jitter and −0.001 °/min drift, before the sensor fault (issue 2) began.'),
  ...
  P('**Measured cause.** The diagnostic build timed each ~23 ms cycle: ~10.7 ms reading the thigh IMU and ~11.7 ms printing the data line as ~36 separate prints. Sending a longer line as one write took ~0.7 ms.'),
  ...
  bullet('**Fix made, awaiting a test run:** the central now builds each data line in memory and sends it with one write, which should bring the cycle to ~11–12 ms and the rate to a true 50 Hz.'),
  bullet('Still planned: write one CSV row per received sample, so duplicates can\'t happen.'),

  H2('2. The ruler-side node\'s sensor fails after ~20 minutes'),
  P('Seen in two recordings with the boards untouched: a 1-minute no-motion recording, and a ~40-minute unattended run with the diagnostic build (both boards on USB power). The diagnostic run shows the sequence:'),
  table(['Board uptime', 'What the ruler node reported'], [
  ...
  gap(),
  bullet('**Not the link:** 0 checksum failures and 0 corrupted bytes for the whole run, and the ruler board\'s health reports kept arriving.'),
  bullet('**Not a board reset or power loss:** its uptime counter never restarted.'),
  bullet('**Not calibration:** the fault is between the ruler board\'s processor and its own sensor. The desk node\'s sensor stayed perfect for all 41 minutes.'),
  P('The likely cause is a hardware problem on that board (the sensor, its connections, or heat), but a firmware cause isn\'t ruled out yet. Swapping the two boards\' roles will tell: if the fault stays with the same physical board, it\'s that board.', { para: { spacing: { before: 80, after: 120 } } }),
  P('**Firmware protection (made, awaiting a test run):** the ruler board now rejects implausible samples and sends them as gaps instead of wrong angles, resets its filter after each sensor restart, and reboots itself if its sensor produces nothing usable for 5 s.'),
  ...
  bullet('**Mostly 5-minute holds.** One clean 18-minute stretch exists; longer drift isn\'t measured yet because of the sensor fault.'),
